...
<script>
    var parsedJSON;
    if ("<?php echo ($type); ?>" == "WORKSHEAT") {
        Cajax.post("/course/<?php echo ($courseid); ?>/post/<?php echo ($pageid); ?>/submits").then(function (resp) {
    ...
    } else if ("<?php echo ($type); ?>" == "IMAGE_WORKSHEAT") {
        Cajax.post("/course/<?php echo ($courseid); ?>/post/<?php echo ($pageid); ?>/submits:image").then(function (resp) {
            parsedJSON = JSON.parse(resp.responseText);
            if (parsedJSON.done) {
